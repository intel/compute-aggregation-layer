...
constexpr unsigned int ultIterationMaxTime = 45;
constexpr const char *executionName = "ULT";
...
void handleSIGABRT(int signal) {
    ...
    std::cout << "SIGABRT in " << executionName << ", on: " << lastTest << std::endl;
...
void handleSIGALRM(int signal) {
    ...
    std::cout << "Tests timeout in " << executionName << ", ";
...
void handleSIGSEGV(int signal) {
    ...
    std::cout << "SIGSEGV in " << executionName << ", on: " << lastTest << std::endl;
    abort();
}
...
int setAlarm(bool enableAlarm) {
    ...
    if (enableAlarm) {
        std::string envVar = std::string("CAL_") + executionName + "_DISABLE_TEST_ALARM";
    ...
    if (enableAlarm) {
        ...
        auto currentUltIterationMaxTime = ultIterationMaxTime;
        auto ultIterationMaxTimeEnv = getenv("CAL_ULT_ITERATION_MAX_TIME");
...
int enableSignals(bool enableAlarm, bool enableSegv, bool enableAbrt) {
    if (int sigOut = setAlarm(enableAlarm); sigOut != 0) {
        return sigOut;
    }

    if (int sigOut = setSegv(enableSegv); sigOut != 0) {
        return sigOut;
    }
    if (int sigOut = setAbrt(enableAbrt); sigOut != 0) {
        return sigOut;
    }
